...
argumento $s$ que codifica la ``solución parcial'' $(s)$ a mejorar en cada etapa del algoritmo. Veremos 2 soluciones, ambas recursivas, que implementan diferentes podas en sus búsquedas.

\subsubsection{Poda por factibilidad}

El primero que veremos requiere un orden de los ítems previo a instanciarlos, deben estar ordenados por peso de forma creciente.

Entonces al asegurar que para un indice $(i)$ si $\sum_{j = 0}^{i} p[i]_{peso} > W$ entonces todas las sumatorias con indice mayor a $(i)$ también exceden W.
...
\item[] \begin{itemize}
	\item[] $p$,  Los pares ordenados de menor a mayor según su peso
	\item[] $W$, El peso máximo de la mochila
	\item[] $s$ - (solución parcial)
	\begin{itemize}
		\item[] $s_i = 0$ indice actual
		\item[] $s_{valor} = s_{peso} = 0$ inicializa con la mochila vacia \O
...
\State $\textit{con-valor} \is \Call{Knapsack-Optimalidad}{p, W, \textit{ considerar } s_i}$
\Comment $s_i \in mochila$
\State $\textit{sin-valor} \is \Call{Knapsack-Optimalidad}{p, W, \textit{ no considerar } s_i}$
\Comment $s_i \not\in mochila$
\State \Return $\Call{max}{\textit{con-valor}, \textit{sin-valor}}$
\EndFunction
\end{algorithmic}
\begin{description}
	\item[\textbf{Complejidad algorítmica:}] $O(n \times 2^n)$
...
La poda por factibilidad podemos estimar que sus casos favorables rondarán cuando W sea muy pequeño o muy grande en comparación de ($\sum_{i = 0}^{\#p - 1} p[i]_{peso}$).

\textit{ver experimentación}


\subsubsection{Poda por optimalidad}

La segunda poda que veremos no requiere un ordenamiento previo, en su lugar se debe calcularse $\sum_{i = 0}^{\#p - 1}p[i]_{valor}$ la suma de los valores a considerar y ofrecer un candidato ($m$) el cual es una mochila válida.

A la hora de considerar cada ítem podemos utilizar la mochila candidato $m$ y el valor de nuestros hijos para determinar si somos capaces de mejorar $m$\footnote{En caso contrario podemos devolver $m$ ya que sabemos que es una mochila válda.}. En nuestra implementación previo a correr el algoritmo ordenamos los ítems para conseguir un $m$ grande lo antes posible.
...
\item[\textbf{Inicialización:}]
\item[] \begin{itemize}
	\item[] $p$, Los pares (preferiblemente ordenados por valor)
	\item[] $W$, El peso máximo de la mochila
	\item[] $s$ - (solución parcial)
	\begin{itemize}
		\item[] $s_i = \#p - 1$
	...
	\end{itemize}
	\item[] $m = 0$ (la mejor combinación de valores encontrada)
\end{itemize}
\Statex
