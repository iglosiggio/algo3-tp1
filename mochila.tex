...
El algoritmo de programación dinámica hace uso de las mismas técnicas
exploratorias, que los de backtracking. A partir de un elemento realizaremos
...
para no tener que volver a calcularlas luego.
...
\Comment $p[i] \not \in \text{mochila}$
	\State $\text{con} \is p[i]_{peso} + \Call{Knapsack-Dinamica}{i - 1, r - p[i]_{peso}, p}$
\Comment $p[i] \in \text{mochila}$
\State $mem[k][Q] \is \Call{max}{sin, con}$
