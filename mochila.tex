...
\begin{enumerate}
	...
	dado que su complejidad sería $O(2^n)$, no ganaríamos mucho respecto de fuerza bruta).
...
\item[] \begin{itemize}
	...
	\item[] \Call{mochilas}{$p$} Arma resultados con partes de $p$, siendo cada resultado $\sum_{i = 0}^{(\#combinacion -1)} (peso_i, valor_i)$. 
	
	\item[] \Call{ordenar}{$p$} Ordenamiendo por compacación con
	complejidad $O(n\log n)$
	
	\item[] \Call{descartar}{$p$} donde $\forall (par_i)$ con $(0 < i < \#p)$, elimina el par si el $par_{i-1}$ poseé un valor mayor. Requiere $p$ ordenado, complejidad $O(p)$

	\item[] \Call{busqueda}{$p, k$} búsqueda binaria, devuelve la primera aparición que es igual a k, o el valor pequeño más cercano.
\end{itemize}
\Statex
\Function{Knapsack-MeetInTheMiddle}{$p, w$}
\State $Izq$ $\is \Call{mochilas}{p[0\dots \frac{n}{2}]}$
\State $Der$ $\is \Call{mochilas}{p[\frac{n}{2}\dots n]}$
...
\For{$(a \in Izq)$}
	\If{$a_{peso} \leq w$}
	\State $b \is \Call{busqueda}{Der, w - a_{peso}}$
