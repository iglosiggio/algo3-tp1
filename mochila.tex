...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.9\textwidth]{{fotos/pdf.dinamica}.pdf}
...
algoritmo de programación dinámica construimos decidimos construir casos de
test mucho más grandes. Los peores casos posibles eran los de $n = W$ por lo
que decidimos utilizar muchos de éstos y esperar un crecimiento cuadrático.
...
Hemos considerado como resolver el problema de la mochila con diferentes métodologias, así como hemos buscado identificar los tiempos de complejidad y memoria requerida e aplicado heúristicas que busquen podar y reducir el espacio de nuestro problema.

Tras someter a nuestros algoritmos a extensas pruebas. Los resultados mas destacables son los siguientes:
\begin{enumerate}
	\item La poda por optimalidad debido al ordenamiento por valores que le permite armar rapidamente un buen candidato a la mejor mochila, muestra un rendimiento muy superior que otros algoritmos como Meet in the Middle el cual posee una complejidad teórica mejor. 

	\item El algoritmo de memoria dinámica tuvo un rendimiento extremadamente rapido en comparación con los otros algoritmos, mantuvo un consumo de memoria moderado mejor a fuerza bruta y Meet in the Middle que requieren armar todo el espacio de soluciones y muy inferior a los algoritmos de backtracking que realizan búsquedas exploratorias y poseen un consumo despreciable.
	
	\item la poda por factibilidad, en el experimento B mostro ser poco efectiva en casos de $n$ grande, lo cual se debe en gran medida a que los experimentos realizados buscaban mantener un $W$ decente para la cantidad respectiva de items con sus valores, Dicha poda solo muestra efectividad para casos en los que W es relativamente bajo o uno excesivamente alto.
...
Referencias:
\begin{itemize}
	\item[] \textit{Computing partitions with applications to the knapsack - Ellis Horowitz and Sartaj Sahni.}
	problem.
	\item[] 
